...
def des(pt, rkb):
	...
	for i in range(0, 16):
		...
		for j in range(0, 8):
			...
			# print(f"round {i} sbox {j}[{row}][{col}]: {val}, key = {rkb[i][:6]}")
...
def select_function(ct, key):
	...
	# Expansion D-box: Expanding the 32 bits data into 48 bits
	right_expanded = permute(right, exp_d, 48)[:6]
	# print(f"right expanded: {permute(right, exp_d, 48)}")

	# XOR RoundKey[i] and right_expanded
	xor_x = xor(right_expanded, key)

	row = bin2dec(int(xor_x[0] + xor_x[5]))
	col = bin2dec(
		int(xor_x[1] + xor_x[2] + xor_x[3] + xor_x[4]))
	val = sbox[0][row][col]
	val = dec2bin(val)
